...
// ]
function _buildArgs(ctx, params, rawArgs, fromContext, parentType) {
    ...
    if (o.params) {
      if (_.isArray(type)) {
        ...
        val = _.map(val, function(v) {
          return _buildArgs(ctx, o.params, v, false, parentType);
        });
      } else {
        val = _buildArgs(ctx, o.params, val, false, type);
      }
    }
...
module.exports = class Context {
  ...
  buildArgs() {
    ...
    // Build args
    let args = _buildArgs(ctx, params, this.args, true);
