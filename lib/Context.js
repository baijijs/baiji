...
module.exports = class Context extends EventEmitter {
  constructor(req, res, action, options) {
    ...
    // Request reference
    this.request = this.req = Object.create(req);
    // Response reference
    this.response = this.res = Object.create(res);
  ...
  // Inject properties into context instance
  injectProps(obj) {
    _.map(obj || {}, (value, property) => {
      assert(
        !this[property],
        `Context: property conflict detected: ${property}`
      );
  ...
  async buildArgs() {
    ...
    const schema = ctx.action.schema;
    let args;

    if (schema) args = await schema.parse(ctx);
