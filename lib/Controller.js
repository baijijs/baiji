...
const route = function route(proto, nameOrRoutes, methodConfig) {
  ...
  if (typeof nameOrRoutes === 'string') {
    ...
    if (typeof methodConfig === 'object') {
      proto.__routes = Object.assign(
        proto.__routes,
        { [nameOrRoutes]: methodConfig }
      );
    }
...
class Controller extends EventEmitter {
  ...
  constructor(name) {
    ...
    // Combine routes from prototype.__routes
...
RESERVED_METHODS = Object.getOwnPropertyNames(
  Controller.prototype
).filter(name => !~['constructor', 'initRoutes'].indexOf(name));
