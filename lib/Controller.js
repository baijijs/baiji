...
function configure(proto, nameOrConfigs, methodConfig) {
...
class Controller extends EventEmitter {
  ...
  __skipHook(type, name, skipOptions) {
    ...
    let onlies = [];
    _.each(_.get(hook, 'options.only'), function(name) {
    ...
    let excepts = [];
    _.each(_.get(hook, 'options.except'), function(name) {
      if (~skipOptions.except.indexOf('*') || ~skipOptions.except.indexOf(name)) {
        return;
      } else {
        excepts.push(name);
  ...
  */
  ...
  __optionParser(options, prevOpts) {
    ...
    options.only = options.only.concat(_.get(prevOpts, 'only') || []);
    options.except = options.except.concat(_.get(prevOpts, 'except') || []);
