...
class Method extends EventEmitter {
    ...
    function _buildParams(params, index) {
      ...
      _.each(params, function(param, i) {
        ...
        _.each(REQUIRED_PARAM_KEYS, function(key) {
          if (!param[key]) {
            errorMessages.push(
              `\`${key}\` is missing for params of \`${fullName}\` method at position ${index}[${i}]`
            );
          }
        });
  ...
  compose(beforeStack, afterStack, afterErrorStack) {
    ...
    // Error handler
    let afterError;
    if (afterErrorStack.length) afterError = utils.compose(afterErrorStack);
