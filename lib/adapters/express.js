...
module.exports = class ExpressAdapter extends Adapter {
  ...
  createHandler() {
    ...
    // Sort actions, keep the middleware method in front of all other actions
    let middlewareActions = [];
    let normalActions = [];
    _.each(this.actions, function(method) {
      if (method.verb === 'use') {
        middlewareActions.push(method);
      } else {
        normalActions.push(method);
      }
    });
    ...
    _.each(this.sortedActions, function(route) {
      ...
      if (route.upload) {
        ...
        assert(
          ...
          `Missing upload dest or storage config for method ${route.name}`
        );
