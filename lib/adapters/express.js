...
module.exports = class ExpressAdapter extends Adapter {
  ...
  createHandler() {
    ...
    const actionWrapper = (action, route) => {
      let isMiddleware = route.verb === 'use';
      ...
      return (req, res, next) => {
        ...
        return action.invoke(ctx, function (ctx) {
          // Handle error, transfer error to express
          if (!ctx.errorHandled && ctx.error) return next(ctx.error);
    ...
    const sortedRoutes = this.sortRoutes(this.createRoutesBy(actionWrapper));

    // Apply all actions
    _.each(sortedRoutes, function(route) {
      assert(
        router[route.verb],
        `Invalid route verb '${route.verb}' detected for action ${route.name}`
      );
