...
class ExpressContext extends Context {
  ...
  buildArgs(accepts) {
    ...
    for (i = 0; i < accepts.length; i++) {
      ...
      if (httpFn && typeof httpFn === 'function') {
        ...
        val = ctx.param(name, o);
        if (Normalizer.canConvert(o.type)) {
          val = Normalizer.convert(val, o.type, ctx.options);
        } else {
          val = Normalizer.convert(val, 'any', ctx.options);
        }
      }
  ...
  param(name) {
    ...
    var accepts = ctx.accepts(this.supportedTypes);
