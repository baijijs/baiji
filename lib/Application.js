...
class Application extends EventEmitter {
  ...
  define(method) {
    if (method instanceof Method) {
      ...
      this.methods.push(Method.apply(null, arguments));
    }
    return this;
  }
  ...
  clone() {
    ...
    app.beforeActions = this.beforeActions.slice();
    app.afterActions = this.afterActions.slice();
    app.afterErrorActions = this.afterErrorActions.slice();
  ...
  _useFunctionMiddleware(fn, options) {
    ...
    options = options || { mountPath: '/', name: null };

    let name = options.name || utils.getName(fn);

    let method = Method(name, options, function(ctx, next) {
  ...
  _composeMethodStack(method) {
    ...
    let beforeHooks = utils.filterHooks(this.beforeHooks, name);
    let afterHooks = utils.filterHooks(this.afterHooks, name);
    let afterErrorHooks = utils.filterHooks(this.afterErrorHooks, name);
  ...
  adapter(name, options) {
    ...
    let handler = adapter.createHandler();
