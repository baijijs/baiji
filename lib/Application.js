...
class Application extends EventEmitter {
  ...
  _useController(ctrl, options) {
    ...
    _.each(ctrl.__configs, function(methodConfig, methodName) {
      // Allow to use custom method to invoke api call
      let _method = methodConfig.method || methodName;

      assert(
        typeof ctrl[_method] === 'function',
        `No method named '${_method}' defined for Controller '${name}'`
      );

      let methodFn = utils.wrapperFn(ctrl[_method]);
