...
class Application extends EventEmitter {
  ...
  _useController(ctrl, options) {
    ...
    _.each(ctrl.__configs, function(actionConfig, actionName) {
      // Allow to use custom action to invoke api call
      let action = actionConfig.action || actionConfig.method  || actionName;

      assert(
        typeof ctrl[action] === 'function',
        `No action named '${action}' defined for Controller '${name}'`
      );

      let actionFn = utils.wrapperFn(ctrl[action]);
  ...
  composedActions() {
    ...
    return _.map(this.allActions(), function(action) {
      ...
      action.compose({
        before: utils.filterHooks(beforeHooks, name),
        after: utils.filterHooks(afterHooks, name),
        error: utils.filterHooks(afterErrorHooks, name),
        beforeRespond: utils.filterHooks(beforeRespondHooks, name)
      });
