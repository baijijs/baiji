...
class Application extends EventEmitter {
  ...
  _useFunctionMiddleware(fn, options) {
    ...
    options.http = options.http || { path: options.mountpath || '/', verb: 'all' };
  ...
  _useApplication(app, options) {
    ...
    this.methods = this.methods.concat(app.methods);
  ...
  composedMethods() {
    return this.methods.map((method) => this._composeMethodStack(method));
  }
