...
class Application extends EventEmitter {
  ...
  // Search all methods by 
  searchAllMethodsByName(pattern, callback) {
    let cb = typeof callback === 'function' ? callback : function(m) { return m; };

    let selectedMethods = [];

    _.each(this.allMethods(), function(method) {
      if (mm.isMatch(method.fullName(), pattern)) {
        selectedMethods.push(cb(method));
      }
    });
